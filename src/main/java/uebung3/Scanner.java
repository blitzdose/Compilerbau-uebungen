...
public class Scanner {
    ...
    public List<Token> scanLine(String line, int lineNumber) {
        ...
        while (!deque.isEmpty()) {
            ...
            String possibleToken = buildTokenFromMemory(memory);
            if (!possibleToken.isEmpty() && possibleToken.chars().allMatch(Character::isDigit)) {
                while (!deque.isEmpty() && (Character.isDigit(deque.peek()) || deque.peek() == '.')) {
                    ...
                    if (Character.isDigit(c)) {
                        ...
                        tokens.add(new Token(TokenType.NUMBER, buildTokenFromMemory(memory), buildTokenFromMemory(memory), lineNumber));
                        memory.clear();
                        memory.add(c);
                    }
                }
                possibleToken = buildTokenFromMemory(memory);
