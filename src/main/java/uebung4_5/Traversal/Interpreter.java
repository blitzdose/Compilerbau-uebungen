...
import uebung3.Parser.ParserError;
import uebung3.Parser.Stmts.*;
import uebung4_5.Traversal.InterpreterUtils.*;
...
public class Interpreter implements ExprVisitor<Object>,
    ...
    public Object visitBinaryExpr(Binary expr) {
        ...
        switch (expr.operator.type) {
            ...
            case SLASH:
                if ((double) right == 0) {
                    throw new RuntimeError(expr.operator, "Divide by zero not allowed.");
                }
                return (double) left / (double) right;
            default:
                break;
    ...
    public Object visitCallExpr(Call expr) {
        if (this.environment.get(((Variable) expr.callee).name) instanceof LoxFunction fun) {
            return fun.call(this, expr.arguments.stream().map(Object.class::cast).collect(Collectors.toList()));
        }
        if (this.environment.get(((Variable) expr.callee).name) instanceof Interpreter interpreter) {
            return ((LoxCallable) interpreter.globals.get(((Variable) expr.callee).name)).call(interpreter, null);
        }
        return null;
    }
